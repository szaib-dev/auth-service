  ...
  afterAll(async () => {
   ...
   describe(' Good Requests ', ()=>{
    ...
    it('should check accessToken and refreshToken in cookies',async()=>{
          ...
          const cookies = (response.headers as unknown as {'set-cookie': string[]})['set-cookie'] ?? []
          
          
         const accessToken = cookies.find(cookie => cookie.startsWith('accessToken'))?.split(";")[0].split("=")[1] ?? null
         const refreshToken = cookies.find(cookie => cookie.startsWith('refreshToken'))?.split(";")[0].split("=")[1] ?? null
